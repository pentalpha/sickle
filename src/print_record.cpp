...
void print_record (FILE *fp, FQEntry &fqr, cutsites *cs) {
    fprintf(fp, "%s", string(fqr.name).c_str());
    if (fqr.comment.length()) fprintf(fp, " %s\n", string(fqr.comment).c_str());
    else fprintf(fp, "\n");
    fprintf(fp, "%s\n", string(fqr.seq.substr(cs->five_prime_cut, cs->three_prime_cut - cs->five_prime_cut)).c_str());
    fprintf(fp, "+\n");
    fprintf(fp, "%s\n", string(fqr.qual.substr(cs->five_prime_cut, cs->three_prime_cut - cs->five_prime_cut)).c_str());
}

void print_record_gzip (gzFile fp, FQEntry &fqr, cutsites *cs) {
    gzprintf(fp, "%s", string(fqr.name).c_str());
